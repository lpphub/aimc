    ...
    <div className='grid grid-cols-1 md:grid-cols-3 gap-6 p-8'>
      {tools.map(tool => {
        ...
        return (
          <Card
            key={tool.id}
            className={`group relative bg-gradient-to-br ${tool.gradient} backdrop-blur-sm border-border/30 hover:scale-[1.02] transition-all duration-300 cursor-pointer overflow-hidden`}
            onClick={() => onSelect(tool.id)}
          >
            <div className='p-8'>
              <div className='w-14 h-14 rounded-2xl bg-gradient-to-br from-foreground/10 to-foreground/5 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform'>
